...
public class BoardSearchImpl extends QuerydslRepositorySupport implements BoardSearch {
    ...
    @Override
    public Page<BoardListAllDTO> searchAllTables(String[] types, String keyword, Pageable pageable) {
        ...
        query.groupBy(board);   // 추가

        // 페이징 내장 함수가 있어서 QuerydslRepositorySupport 사용
        getQuerydsl().applyPagination(pageable,query);

        JPQLQuery<Tuple> tupleQuery = query.select(board, reply.countDistinct());   // 추가

        List<Tuple> list = tupleQuery.fetch();
        List<BoardListAllDTO> dtoList = list.stream().map(tuple -> {
            ...
            List<BoardImageDTO> imgDto = board1.getImageSet().stream().sorted()
